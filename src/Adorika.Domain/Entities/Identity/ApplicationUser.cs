...
public class ApplicationUser : IdentityUser<Guid>
{
    ...
    /// This is REQUIRED - ApplicationUser is for tenant-scoped users only.
    /// For platform-level SuperAdmins, use PlatformUser instead.
        ...
        string lastName)
    {
        return string.IsNullOrWhiteSpace(tenantId)
            ? throw new ArgumentException("TenantId is required for ApplicationUser. Use PlatformUser for platform-level users.", nameof(tenantId))
            : new ApplicationUser
        {
            Id = Guid.NewGuid(),
            TenantId = tenantId,
